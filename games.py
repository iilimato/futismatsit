...
def count_games():
    sql = "SELECT COUNT(*) count FROM games"
    result = db.query(sql)
    return result[0]["count"]


def get_games(page, page_size):
...
def count_games_by_query(query):
    sql = """SELECT COUNT(*) count FROM games
...
def find_games_by_query(query, page, page_size):
    sql = """SELECT games.id, games.title, games.description,
             games.date, games.time, games.location,
             games.player_count, users.username
             FROM games LEFT JOIN users
    ...
    like_query = f"%{query}%"
    offset = page_size * (page - 1)
    return db.query(sql, [like_query, like_query, like_query, page_size, offset])
